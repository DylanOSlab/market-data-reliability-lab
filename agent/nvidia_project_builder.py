...
def repository_context(work: dict[str, Any]) -> str:
    ...
    candidates.sort(
        key=lambda p: (0 if p.as_posix().startswith(("src/", "tests/")) else 1, p.as_posix())
    )
...
def is_duplicate(plan: dict[str, Any], work: dict[str, Any]) -> bool:
    ...
    if not candidate:
        return True
    titles = [
        normalize_title(str(item.get("title", ""))) for group in work.values() for item in group
    ]
...
def preservation_check(plan: dict[str, Any]) -> None:
    for item in plan["files"]:
        ...
        old_defs = set(re.findall(r"^(?:def|class)\s+([A-Za-z_]\w*)", old, flags=re.MULTILINE))
        new_defs = set(re.findall(r"^(?:def|class)\s+([A-Za-z_]\w*)", new, flags=re.MULTILINE))
...
def repair(plan: dict[str, Any], failure: str) -> dict[str, Any]:
    current = [
        {"path": item["path"], "content": Path(item["path"]).read_text(encoding="utf-8")}
        for item in plan["files"]
    ...
    prompt = {
        ...
        "validation_output": failure[-16000:],
        "instruction": "Return the same JSON schema with corrected complete files. Preserve scope, acceptance criteria, and unrelated content.",
    }
    return call_nim(
...
def main() -> None:
    ...
    if not baseline_ok:
        raise RuntimeError(
            f"Baseline checks fail; autonomous coding stopped\n{baseline_output[-6000:]}"
        )
